...
function KpiCard({ label, value, sub, icon: Icon, color }) {
  return (
    <div
      className={`rounded-2xl p-5 border border-slate-100 bg-white shadow-sm flex gap-4 items-start`}
    >
      <div
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex-1 px-4 py-2 bg-primary-600 text-white rounded-xl text-sm font-semibold hover:bg-primary-700 disabled:opacity-50"
          >
...
function PlanBadge({ plan }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${PLAN_COLORS[plan] || "bg-slate-100 text-slate-600"}`}
    >
...
function DocSection({ title, icon: Icon, iconColor, children }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      <div
        className={`flex items-center gap-3 px-6 py-4 border-b border-slate-100 ${iconColor || "bg-slate-50"}`}
      >
...
function Code({ children, label, lang = "graphql" }) {
  return (
    <div className="rounded-xl overflow-hidden border border-slate-200 my-3">
      ...
      <pre className="bg-slate-900 text-emerald-300 text-xs font-mono p-4 overflow-x-auto leading-relaxed whitespace-pre">
        {children}
      </pre>
    </div>
  );
...
function Pill({ label, color }) {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-semibold ${colors[color] || "bg-slate-100 text-slate-600"}`}
    >
      {label}
    </span>
  );
  ...
  return (
    <div className="border border-slate-100 rounded-xl p-4 space-y-2 hover:border-slate-200 transition-colors">
      <div className="flex items-center flex-wrap gap-2">
        ...
        <Pill
          label={auth}
          color={auth === "superadmin" ? "superadmin" : "auth"}
        />
      </div>
        ...
        <div className="text-xs text-slate-500">
          ...
          {params.map((p, i) => (
            <span key={i}>
              ...
              : {p.type}{" "}
              {p.required ? (
                <Pill label="required" color="required" />
              ) : (
                <Pill label="optional" color="optional" />
              )}
              {i < params.length - 1 ? ", " : ""}
            </span>
          ))}
...
export default function SuperAdminPage() {
  ...
  const {
    ...
    loading: dashLoading,
    refetch: refetchDash,
  } = useQuery(SUPER_ADMIN_DASHBOARD);
  const {
    data: tenantsData,
    ...
    variables: {
      ...
      search: searchTenants || undefined,
    },
    errorPolicy: "all",
  });
              ...
              <>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  <KpiCard
                    ...
                    value={dash?.totalTenants ?? 0}
                    icon={Building2}
                    color="bg-purple-100 text-purple-600"
                  />
                  <KpiCard
                    label="Active"
                    value={dash?.activeTenants ?? 0}
                  ...
                  <KpiCard
                    ...
                    value={`₹${((dash?.totalRevenueAcrossPlatform ?? 0) / 1000).toFixed(0)}K`}
                    sub={`${dash?.totalBookingsAcrossPlatform ?? 0} bookings`}
                    icon={DollarSign}
                ...
                {(dash?.suspendedTenants ?? 0) > 0 && (
                  <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm">
                    ...
                    <span>
                      <strong>{dash.suspendedTenants}</strong> companies
                      currently suspended.
                    </span>
                  </div>
                )}
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                    ...
                    <div className="space-y-3">
                      {(dash?.planBreakdown || []).map((p) => {
                        const pct =
                          dash.totalTenants > 0
                            ? Math.round((p.count / dash.totalTenants) * 100)
                            : 0;
                        const colorBar =
                          p.plan === "enterprise"
                            ? "bg-purple-500"
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-100">
              ...
              <p className="text-sm text-slate-400 mt-0.5">
                Recent events across all companies
              </p>
            </div>
            <div className="divide-y divide-slate-50">
              {activityLog.map((a) => (
                  ...
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                      a.action.includes("CREATED")
                        ...
                        : a.action.includes("SUSPENDED")
                          ? "bg-red-100 text-red-600"
                          : "bg-slate-100 text-slate-500"
                    }`}
                  >
                    {a.action.includes("TENANT") ? (
        ...
        {activeTab === "docs" && (
          <div className="space-y-6">
            ...
            <DocSection
              ...
              icon={Layers}
              iconColor="bg-gradient-to-r from-violet-50 to-indigo-50"
            >
              <p className="text-sm text-slate-600 mb-4">
                TrailDesk is a <strong>multi-tenant SaaS</strong> built on a
                shared-database model. A single MongoDB cluster stores all
                tenant data — each document is tagged with a{" "}
                <code className="bg-slate-100 px-1 rounded text-violet-700">
                  tenantId
                </code>{" "}
                that acts as the invisible row-level security filter on every
                query.
              </p>
              ...
              <div className="bg-slate-900 rounded-xl p-5 font-mono text-xs leading-6 overflow-x-auto text-slate-300 mb-4">
                ...
                <div>
                  {" "}
                  │ authMiddleware → decodes JWT → req.user = {"{"} userId,
                  tenantId, role {"}"}
                </div>
                <div> ▼</div>
                <div className="text-emerald-300">
                  GraphQL Engine (graphql-http)
                </div>
                <div>
                  {" "}
                  │ context = {"{"} user: req.user {"}"} passed to every
                  resolver
                </div>
                <div> ▼</div>
                <div className="text-pink-300">
                  Resolvers (per domain module)
                </div>
                <div> │ requireAuth(context) → throws if no JWT</div>
                <div>
                  {" "}
                  │ filter = {"{"} tenantId: user.tenantId {"}"} → tenant
                  isolation
                </div>
                <div> ▼</div>
                ...
                <div className="text-slate-400">
                  Response → Apollo Cache → React UI
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4">
                {[
                  ...
                  <div
                    key={layer.label}
                    className={`rounded-xl border p-4 ${layer.color}`}
                  >
                    <div
                      className={`flex items-center gap-2 mb-3 font-bold text-sm ${layer.tc}`}
                    >
            ...
            {/* ── MULTI-TENANCY MODEL ── */}
            <DocSection
              title="Multi-Tenancy: How Data Isolation Works"
              icon={Database}
              iconColor="bg-emerald-50"
            >
              <p className="text-sm text-slate-600 mb-3">
                All companies share the{" "}
                <strong>same MongoDB collections</strong>. Every document has a{" "}
                <code className="bg-slate-100 px-1 rounded">tenantId</code>{" "}
                field referencing the{" "}
                <code className="bg-slate-100 px-1 rounded">Tenant</code> model.
                Queries always filter by the tenant extracted from the JWT token
                — company data never crosses boundaries.
              </p>
                ...
                <div>
                  ...
                  <div className="bg-slate-900 rounded-xl p-4 font-mono text-xs leading-6">
                    ...
                    <div className="text-amber-300">
                      {'{"_id":"...", "tenantId":"A", "amount":5000}'}
                    </div>
                    <div className="text-blue-300">
                      {'{"_id":"...", "tenantId":"B", "amount":8000}'}
                    </div>
                    <div className="text-amber-300">
                      {'{"_id":"...", "tenantId":"A", "amount":3500}'}
                    </div>
                    <div className="text-slate-500 mt-2">
                      # Company A only sees their rows
                    </div>
                    <div className="text-emerald-300">
                      {'Booking.find({ tenantId: "A" })'}
                    </div>
                  </div>
                </div>
                <div>
                  <p className="text-xs font-semibold text-slate-500 mb-2 uppercase tracking-wide">
                    Resolver pattern (every query)
                  </p>
...
const docs = await Model.find({
  tenantId: user.tenantId  // enforced!
              ...
              <div className="mt-4 p-4 bg-violet-50 border border-violet-200 rounded-xl">
                <p className="text-xs font-semibold text-violet-700 mb-1">
                  Collections with tenantId isolation:
                </p>
                <div className="flex flex-wrap gap-2 mt-1">
                  {[
                    ...
                    <code
                      key={m}
                      className="bg-white border border-violet-200 text-violet-700 text-xs px-2 py-0.5 rounded-lg"
                    >
                      {m}
                    </code>
                  ))}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {[
                      [
                        "1",
                        "User sends phone + password to",
                        "mutation login(...)",
                      ],
                      [
                        "2",
                        "Resolver finds User by phone, verifies bcrypt hash",
                        "",
                      ],
                      [
                        "3",
                        "Checks Tenant status (active/trial — rejects suspended)",
                        "",
                      ],
                      [
                        "4",
                        "Signs JWT with",
                        "{ userId, tenantId, role } · 7d expiry",
                      ],
                      [
                        "5",
                      ...
                      [
                        "8",
                        "Resolver calls requireAuth(context) → gets user object",
                        "",
                      ],
                    ].map(([n, a, b]) => (
                      <div key={n} className="flex items-start gap-3 text-sm">
                        ...
                        <span className="text-slate-600">
                          {a}{" "}
                          <code className="bg-slate-100 px-1 rounded text-slate-800 text-xs">
                            {b}
                          </code>
                        </span>
                      </div>
  ...
  "exp":      1772834070    // 7 days
                  ...
                  <p className="text-xs font-semibold text-slate-500 mb-2 uppercase tracking-wide mt-4">
                    Role hierarchy
                  </p>
                  <div className="space-y-2">
                    {[
                      {
                        role: "superadmin",
                        desc: "Platform owner only. Can manage all tenants. No tenantId in JWT.",
                        color: "bg-purple-100 text-purple-700",
                      },
                      {
                        role: "admin",
                        desc: "Full access to one tenant. Created when a company registers.",
                        color: "bg-blue-100 text-blue-700",
                      },
                      {
                        role: "staff",
                        desc: "Limited access within a tenant. Future feature.",
                        color: "bg-slate-100 text-slate-600",
                      },
                    ].map((r) => (
                      <div key={r.role} className="flex items-start gap-2">
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-semibold ${r.color} flex-shrink-0`}
                        >
                          {r.role}
                        </span>
            ...
            <DocSection
              title="Super Admin GraphQL Reference"
              icon={GitBranch}
              iconColor="bg-violet-50"
            >
              <p className="text-sm text-slate-500 mb-4">
                All queries and mutations below require a valid JWT with{" "}
                <code className="bg-slate-100 px-1 rounded">
                  role: superadmin
                </code>
                . Regular tenant admins receive a{" "}
                <code className="bg-slate-100 px-1 rounded">
            ...
            {/* ── EXAMPLE PAYLOADS ── */}
            <DocSection
              title="Example Queries & Payloads"
              icon={BookOpen}
  ...
  superAdminDashboard {
    ...
    recentSignups { _id name plan status createdAt }
                ...
                <Code
                  lang="graphql"
                  label="Create a new company + admin"
                >{`mutation {
  createTenant(input: {
    name: "Summit Treks",
    slug: "summit-treks",
    ...
    _id name plan status adminEmail
                ...
                <Code
                  lang="graphql"
                  label="Tenant dashboard (admin user)"
                >{`query {
  getDashboard {
    kpis {
    ...
    alerts { type message }
                ...
                <Code
                  lang="json"
                  label="Typical JWT Authorization header (send with every request)"
                ...
                {[
                  {
                    ...
                    hdr: "bg-slate-100 text-slate-700",
                    features: [
                      "1 admin user",
                  ...
                  >
                    <div
                      className={`flex items-center gap-2 px-4 py-3 font-bold text-sm capitalize ${p.hdr}`}
                    >
            ...
            <DocSection
              ...
              icon={Star}
              iconColor="bg-gradient-to-r from-amber-50 to-orange-50"
            >
              <div className="space-y-3">
                {[
                  {
                    ...
                    status: "planned",
                    desc: "One-click login via Google. Requires Google Client ID in .env. Library: @react-oauth/google.",
                  },
                  {
                    phase: "Phase 3",
                    label: "Real OTP (SMS)",
                    status: "planned",
                    desc: "Replace mock OTP with Twilio / Msg91. Backend: /api/send-otp + /api/verify-otp endpoints.",
                  },
                  {
                    phase: "Phase 4",
                    label: "Licence Expiry Enforcement",
                    status: "planned",
                    desc: "Auto-suspend tenants whose licenseExpiry has passed. Cron job or check on login.",
                  },
                  {
                    phase: "Phase 4",
                    label: "Audit Log Model",
                    status: "planned",
                    desc: "Replace in-memory activity log with a real AuditLog MongoDB model storing every mutation.",
                  },
                  {
                    phase: "Phase 4",
                    label: "Email Notifications",
                    status: "planned",
                    desc: "Send booking confirmations, payment receipts and invoice PDFs via Nodemailer / SendGrid.",
                  },
                  {
                    phase: "Phase 5",
                    label: "White-labelling",
                    status: "future",
                    desc: "Per-tenant logo, colour palette, and custom subdomain (tenant.traildesk.app).",
                  },
                  {
                    phase: "Phase 5",
                    label: "Staff Role + Permissions",
                    status: "future",
                    desc: "Granular permissions per staff member — read-only, bookings-only, etc.",
                  },
                  {
                    phase: "Phase 5",
                    label: "Billing Integration",
                    status: "future",
                    desc: "Razorpay Subscriptions for automatic plan renewals + invoice generation.",
                  },
                  {
                    phase: "Phase 6",
                    label: "Mobile App",
                    status: "future",
                    desc: "React Native / Expo app sharing the same GraphQL backend.",
                  },
                  {
                    phase: "Phase 6",
                    label: "Analytics Dashboard",
                    status: "future",
                    desc: "Revenue trends, customer retention, booking funnels — per tenant and platform-wide.",
                  },
                ].map((item) => (
                  <div
                    key={item.label}
            ...
            <DocSection
              title="Mandatory Fields Checklist"
              icon={AlertTriangle}
              iconColor="bg-red-50"
            >
              <p className="text-sm text-slate-500 mb-4">
                Every time you add a new collection or feature, these fields{" "}
                <strong>must</strong> be included to maintain multi-tenancy and
                data integrity.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  {[
                    [
                      "tenantId",
                      "ObjectId ref Tenant, index: true",
                      "critical",
                    ],
                    [
                      "timestamps: true",
                      "Always include in schema options",
                      "critical",
                    ],
                ...
                <div className="space-y-2">
                  ...
                  {[
                    [
                      "requireAuth(context)",
                      "First line of every resolver",
                      "critical",
                    ],
                    [
                      "{ tenantId: user.tenantId }",
                      "Every find/findOne/findById query",
                      "critical",
                    ],
                    [
                      "findOneAndUpdate with tenantId",
                      "Mutations — never findByIdAndUpdate alone",
                      "critical",
                    ],
                    [
                      "errorPolicy: all",
                      "Apollo queries — graceful degradation",
                      "recommended",
                    ],
                  ].map(([field, desc, level]) => (
                    <div key={field} className="flex items-start gap-2 text-xs">
